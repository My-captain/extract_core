...
import PyDocExtractor

LICENSE_URL = ('https://raw.githubusercontent.com/NLPIR-team/NLPIR/master/License/license'
               '%20for%20a%20month/DocExtractor%E6%96%87%E6%A1%A3%E6%8F%90%E5%8F%96%E6%8E%88%E6%9D%83/'
               'DocExtractor.user')
DATA_DIR = os.path.join(PyDocExtractor.pydocextractor.PACKAGE_DIR, 'Data')
LICENSE_FILENAME = 'DocExtractor.user'


@click.group(context_settings=dict(help_option_names=['-h', '--help']),
             options_metavar='[<options>]',
             subcommand_metavar='<command> [<args>]')
@click.version_option(PyDocExtractor.__version__, message='%(prog)s %(version)s')
def cli():
    """A simple command-line interface for DocExtractPy."""
    pass
...
@click.option('-d', '--data-dir', help='The DocExtractorPy data directory to use.',
              type=click.Path(exists=True, file_okay=False, writable=True),
              default=DATA_DIR)
def update(data_dir):
    """Update DocExtractorPy license."""
    try:
