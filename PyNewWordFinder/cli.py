...
import PyNewWordFinder

LICENSE_URL = ('https://raw.githubusercontent.com/NLPIR-team/NLPIR/master/License/'
               'license%20for%20a%20month/NewWordFinder%E6%96%B0%E8%AF%8D%E5%8F%91%E7%8E%B0%E6%8E%88%E6%9D%83/'
               'NewWordFinder.user')
DATA_DIR = os.path.join(PyNewWordFinder.pynewwordfinder.PACKAGE_DIR, 'Data')
LICENSE_FILENAME = 'NewWordFinder.user'


@click.group(context_settings=dict(help_option_names=['-h', '--help']),
             options_metavar='[<options>]',
             subcommand_metavar='<command> [<args>]')
@click.version_option(PyNewWordFinder.__version__, message='%(prog)s %(version)s')
def cli():
    """A simple command-line interface for NewWordFinder."""
    pass


def update_license_file(data_dir):
    """Update DocExtractor license file if it is out-of-date or missing.

    :param str data_dir: The DocExtractor data directory that houses the license.
...
@click.option('-d', '--data-dir', help='The PyKeyExtractor data directory to use.',
              type=click.Path(exists=True, file_okay=False, writable=True),
              default=DATA_DIR)
def update(data_dir):
    """Update PyNewWordFinder license."""
    try:
